...
class Resource {
  ...
  static all (config) {
    ...
    let qs = util.getDataFromArgument(args);
    let path = util.expandPath(this.path, args);

    return this.request(config, mappings.all, path, qs);
  }
  static create (config) {
    let args = Array.from(arguments).splice(1);
    let data = util.getDataFromArgument(args);
    let path = util.expandPath(this.path, args);
    return this.request(config, mappings.create, path, {}, data);
  }
  ...
  static destroy (config) {
    ...
    return this.request(config, mappings.destroy, path, {}, data);
  }
